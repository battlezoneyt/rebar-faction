...
export function useFactionFunctions() {
    ...
    async function addPlayerToFactionBlips(player: alt.Player, characterId: number, factionId: string) {
        ...
        const selfBlip = Rebar.controllers.useBlipGlobal({
            pos: player.pos,
    ...
    async function removePlayerFromFactionBlips(player: alt.Player, characterId: number, factionId: string) {
        if (!player || !player.valid) {
