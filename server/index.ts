...
const POLICE_FACTION = '6664a430ac28cf8f46b826e7';
...
export async function syncJob(player: alt.Player, factionId: string) {
    try {
...
async function updateFactionMembers(factionId: string) {
    try {
        const factionData = await factionUpdate.findFactionById(factionId);
        if (!factionData.members) return;
...
Rebar.events.useEvents().on('character-bound', (player: alt.Player, document: Character) => {
    if (!document.faction) return;
    oldMembers.push({ _id: document._id, faction: document.faction });
